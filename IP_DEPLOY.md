...
```bash
# 替换为你的公网IP
curl http://<你的公网IP>:34567/api/health
```
...
3. 添加环境变量：
   ...
   - **Value**: `http://<你的公网IP>:34567/api`
   - **Environment**: 选择所有环境

**示例**：
```
VITE_API_BASE_URL=http://123.45.67.89:34567/api
```
...
- 检查路由器端口转发
- 确认服务器正在运行
- 使用 `netstat -an | grep 34567` 检查端口是否监听
